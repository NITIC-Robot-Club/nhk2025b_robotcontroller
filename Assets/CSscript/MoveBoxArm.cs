    ...
    public class MoveBoxArm : MonoBehaviour
    {
        ...
        void Update()
        {
            expand[0] = unitySubscriber.boxArmState.Expand[0];
            expand[1] = unitySubscriber.boxArmState.Expand[1];
            height[0] = unitySubscriber.boxArmState.Height[0];
            height[1] = unitySubscriber.boxArmState.Height[1];
            strong[0] = unitySubscriber.boxArmState.Hand_position[0];
            strong[1] = unitySubscriber.boxArmState.Hand_position[1];
            leftText.SetText($"Left Arm - \n  Expand: {expand[0]}\n  Height: {height[0]}\n  Hand Position: {strong[0]}");
            rightText.SetText($"Right Arm - \n  Expand: {expand[1]}\n  Height: {height[1]}\n  Hand Position: {strong[1]}");
